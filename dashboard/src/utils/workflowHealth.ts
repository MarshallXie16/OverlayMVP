 ...
 * 2. Needs Review (status='needs_review' OR success_rate 0.6-0.9)
 * 3. Healthy (status='active' AND success_rate > 0.9)
 * 4. Unknown (draft, processing, archived)
 */
export function calculateWorkflowHealth(workflow: WorkflowListItem): HealthStatus {
  // Broken takes highest priority
  ...
  if (workflow.status === 'active') {
    ...
    // Active but low success rate is broken
    return 'broken';
  }
  ...
  switch (status) {
    ...
    case 'needs_review':
      return {
        ...
        tooltipText: workflow.status === 'needs_review'
    ...
    case 'broken':
      return {
        ...
        tooltipText: workflow.consecutive_failures >= 3
 ...
 */
export function compareByHealth(a: WorkflowListItem, b: WorkflowListItem): number {
