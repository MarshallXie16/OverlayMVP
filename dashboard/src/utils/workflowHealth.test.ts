...
function createMockWorkflow(overrides: Partial<WorkflowListItem>): WorkflowListItem {
  return {
    id: 1,
    company_id: 1,
    created_by: 1,
...
describe('calculateWorkflowHealth', () => {
  ...
  it('should return broken for active workflow with low success rate', () => {
    const workflow = createMockWorkflow({
      status: 'active',
      success_rate: 0.5,
      consecutive_failures: 0,
...
describe('compareByHealth', () => {
  it('should sort broken workflows first', () => {
    const healthy = createMockWorkflow({ status: 'active', success_rate: 0.95 });
  ...
  it('should sort needs_review between broken and healthy', () => {
    const healthy = createMockWorkflow({ status: 'active', success_rate: 0.95 });
    const needsReview = createMockWorkflow({ status: 'needs_review', success_rate: 0.75 });
    const broken = createMockWorkflow({ status: 'broken', success_rate: 0.5 });
