...
export class EventDeduplicator {
  ...
  addEvent(
    ...
    // For clicks on inputs, handle appropriately:
    // - Checkbox/radio: skip click, wait for change event
    // - Text inputs (text, email, password, etc.): skip click, wait for blur event
    if (event.type === "click" && element instanceof HTMLInputElement) {
      ...
      // Skip clicks on text-type inputs - we'll record on blur instead
      ...
      if (textInputTypes.has(element.type)) {
        console.log(
          "[EventDeduplicator] Skipping text input click - will record on blur",
        );
        return;
      }
    }

    // Also skip clicks on textareas - wait for blur
    if (event.type === "click" && element instanceof HTMLTextAreaElement) {
      console.log(
        "[EventDeduplicator] Skipping textarea click - will record on blur",
      );
      return;
    }
    ...
    console.log(
      `[EventDeduplicator] Buffered ${event.type} on ${element.tagName} (priority: ${priority})`,
    );
  ...
  private async flush(recordCallback: RecordCallback): Promise<number> {
    ...
    console.log(
      `[EventDeduplicator] Flushing ${this.pendingEvents.length} buffered events`,
    );
  ...
  private areEventsRelated(
    ...
    if (
      event1.event.type === "submit" &&
      event1.element instanceof HTMLFormElement
    ) {
      if (
        event2.element instanceof HTMLButtonElement &&
        event1.element.contains(event2.element)
      ) {
        return true;
      }
    }
    ...
    if (
      event2.event.type === "submit" &&
      event2.element instanceof HTMLFormElement
    ) {
      if (
        event1.element instanceof HTMLButtonElement &&
        event2.element.contains(event1.element)
      ) {
        return true;
      }
    }
  ...
  private pickBestEvent(group: PendingEvent[]): PendingEvent | null {
    ...
    console.log(
      `[EventDeduplicator] Group of ${group.length} events:`,
